        ...
        <el-form ref="form" :model="form" label-width="80px" :rules="rules">
          ...
          <el-form-item label="课程封面" prop="courseCover" :required="true">
            <div class="flex">
              <el-upload :class="{uoloadSty:showCoverImg,disUoloadSty:noneCoverImg}"
                ...
                :on-preview="handlePictureCardPreview" :on-change="coverChange" :limit="1" :file-list="coverImg">
          ...
          <el-form-item label="课程介绍" prop="courseIntro" :required="true">
            <el-upload :class="{uoloadSty:showIntroImg,disUoloadSty:noneIntroImg}"
              ...
              :on-preview="handlePictureCardPreview" :on-change="introChange" :limit="9" :file-list="introImg">
              <i slot="default" class="el-icon-plus"></i>
            </el-upload>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 回显图片处理
      echoImg() {
        let coverImgList = this.form.courseCover
        let introImgList = this.form.courseIntro.split(',')
        let sectionImgList = this.form.specialCourseSectionList
        sectionImgList.forEach(item => {
          this.sectionImg.push({
            "url": item.sectionUrl
          })
        })
        introImgList.forEach(item => {
          this.introImg.push({
            "url": item
          })
        })
      ...
      handleRemove(file, index) {
        this.form.specialCourseSectionList[index].sectionUrl = ''
        this.medioIsShow = false
      },
      // 大图展示
      ...
      introUpSu(res, file, fileList) {
        ...
        if (res.success === 1) {
          let introObject = Object({
            url: res.data.url
          });
          this.introImg.push(introObject)
        } else {
          this.$message({
            type: 'error',
            message: res.msg
      ...
      // 表单提交
      submitForm() {
        let introImg = this.introImg
        let introArr = introImg.map(item => {
          return item.url;
        })
        this.form.courseIntro = introArr.join(',')
        postCourseEdit(this.form).then((res) => {
          setTimeout(() => {
            this.goBack();
          }, 400);
        });
